...
2. Create a `.env` from the template and fill in the values:
   ...
   `src/config/env.js`. A missing or invalid variable is a fatal configuration
...
├── models/          Media.js — Mongoose schema + indexes
├── repositories/    mediaRepository.js — the ONLY layer that touches Mongoose
├── services/        mediaService.js — the ONLY layer with business logic
├── controllers/     mediaController.js — maps req/res; no Mongoose, no logic
├── routes/          mediaRoutes.js — middleware chains + controller refs only
...
└── app.js           builds the Express app (no listen, no DB)
server.js            connects DB, starts listening, owns process-level handlers
...
- **app.js vs server.js** — `app.js` builds the app; `server.js` owns the
